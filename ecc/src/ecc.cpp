...
void ecc_double(ecc_point*& r, const ecc_point* p, const ecc_param* param) {
    ...
    bn_mod_sub(x3, x3, p->x, param->p, ECC_LEN);       // - x1
    ...
    // --- 结果输出 ---
    // 假设 ecc_make_point 是深拷贝 x3, y3 到 r
    // 如果 r 是结构体指针，通常直接赋值成员
    bn_copy(r->x, x3, ECC_LEN); // 假设你有 bn_copy
